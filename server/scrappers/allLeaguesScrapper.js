    ...
    request(config, (error, response, html) => {
      ...
      if(!error) {
        ...
        $('.card-jogo').each(function () {
          ...
          const date = titleItem.find('.hora-local').attr('content').trim();
          ...
          match.homeTeamScore = scoreItem.find('.placar-mandante').text() || null;

          match.awayTeam = awayTeamItem.find('.nome-completo').text();
          match.awayTeamLogoUrl = awayTeamItem.find('.escudo').attr('src');
          match.awayTeamScore = scoreItem.find('.placar-visitante').text() || null;
...
const _getNextMatches = () => {
  let yesterday = new Date('2018/05/17');
  //yesterday = new Date(yesterday.setDate(yesterday.getDate() -1));
  let endDay = new Date('2018/06/17');
  ...
  return Promise.all(promises).then(data => data.reduce((arr, row) => arr.concat(row)), []);
};
